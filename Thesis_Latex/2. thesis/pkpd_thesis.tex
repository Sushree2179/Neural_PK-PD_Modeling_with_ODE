...
The central contribution is a physics-informed decision pipeline in which predicted molecular endpoints are coupled with a Neural ODE-based PK-PD simulator to support translational analysis. The thesis follows a staged experimental sequence: improvement of molecular representation, integration of real ChEMBL binding data, hERG-focused predictive refinement, probability calibration for safety classifiers, structure-aware joint GNN+MLP fusion modeling, mechanistic PK-PD simulation, Pareto efficacy-safety trade-off analysis, Monte Carlo robustness assessment, and in-silico clinical trial simulation. A RapidDock-inspired, distance-biased transformer branch is also included as an exploratory prototype for future structure-aware extension.

The results show that predictive quality improves materially after real-data integration and fusion modeling, that post-hoc calibration is necessary before using classifier outputs as risk variables, and that the resulting PK-PD framework supports clinically motivated trade-off and trial-level analyses. The thesis concludes by discussing practical limitations, reproducibility considerations, and future directions toward external validation and deployment-oriented decision support.
...
Niniejsza praca przedstawia kompleksowe podejście z zakresu uczenia maszynowego do wspomagania wczesnego rozwoju leków, łączące predykcję właściwości molekularnych z mechanistyczną symulacją farmakokinetyczno-farmakodynamiczną (PK-PD). Zaproponowany przepływ pracy integruje heterogeniczne publiczne zbiory danych, obejmuje inżynierię cech i kontrolę jakości danych oraz wykorzystuje wielozadaniowe modele neuronowe do przewidywania powinowactwa wiązania, kardiotoksyczności związanej z hERG, przepuszczalności Caco-2 oraz klirensu.

Główny wkład pracy stanowi fizycznie ugruntowany pipeline decyzyjny, w którym przewidywane molekularne punkty końcowe są sprzęgnięte z symulatorem PK-PD opartym na Neural ODE w celu prowadzenia analiz translacyjnych. Praca realizuje uporządkowaną sekwencję eksperymentalną obejmującą ulepszenie reprezentacji molekularnej, integrację rzeczywistych danych wiązania z ChEMBL, ukierunkowane doskonalenie predykcji hERG, kalibrację prawdopodobieństw dla klasyfikatorów bezpieczeństwa, modelowanie strukturalne z użyciem wspólnej architektury fuzji GNN+MLP, mechanistyczną symulację PK-PD, analizę kompromisów skuteczność--bezpieczeństwo w ujęciu Pareto, ocenę odporności metodą Monte Carlo oraz symulację badań klinicznych in silico. Dodatkowo uwzględniono eksploracyjną gałąź inspirowaną RapidDock, wykorzystującą transformer z biasem odległościowym jako prototyp przyszłego modelowania uwzględniającego strukturę.
...
\item Develop predictive models for binding affinity, hERG risk, Caco-2 permeability, and clearance, with explicit evaluation of real-data integration and structure-aware fusion modeling.
\item Integrate calibrated model outputs with Neural ODE-based PK-PD simulation for mechanistic and translational analysis.
\item Evaluate decision-oriented behavior through Pareto trade-off analysis, Monte Carlo robustness assessment, and virtual clinical trial simulation.
\end{enumerate}

The key contributions are:
\begin{enumerate}
\item A full experimental pipeline spanning data curation, feature engineering, predictive modeling, calibration, mechanistic simulation, and translational decision analysis.
\item A thesis-essential modeling sequence showing that real binding-data integration, hERG-focused refinement, and joint GNN+MLP fusion produce the strongest predictive basis for downstream PK-PD analysis.
\item A decision-support layer combining Pareto trade-off analysis, Monte Carlo validation, and virtual clinical trial simulation on top of calibrated predictive outputs.
...
The upgrade to 2048-bit real-SMILES fingerprints in the representation-upgrade experiment replaces all zero vectors with Morgan fingerprints computed from canonical ChEMBL SMILES, directly encoding structural information such as:
\begin{itemize}
...
\hline
\end{tabular}
\caption{Representation-upgrade comparison. Upgrading to 2048-bit real SMILES fingerprints introduces structural SAR information absent from the synthetic-FP baseline.}
\label{tab:sec13_upgrade}
\end{table}

\subsection{Real Binding SMILES and ChEMBL Integration}

The real binding integration experiment replaces synthetic placeholder ChEMBL binding data with real binding affinity records (Ki/IC50, expressed as pChEMBL values) from 3,410 unique compounds:
\begin{itemize}
...
\begin{itemize}
  ...
  \item Early stopping: patience = 40 epochs (extended to 60 for the hERG improvement sweep)
...
\hline
\textbf{Experiment} & \textbf{Thesis-essential experiment and role} \\
\hline
Experiment 1 & Real ChEMBL binding SMILES and full 2048-bit fingerprint integration; establishes the transition from synthetic placeholders to a realistic molecular input representation. \\
Experiment 2 & hERG AUROC improvement via hidden\_dim expansion and positive-class weighting; defines the strongest toxicity-classification configuration carried forward into later analysis. \\
...
\section{Real Binding Data Integration}
...
\section{hERG Refinement and Final Fusion Model}
...
\section{Probability Calibration and Robustness}
...
\section{Pareto Efficacy-Safety Trade-Off Analysis}
...
The results chapter supports a coherent interpretation of the proposed neural PK-PD framework as a staged progression from molecular prediction to translational decision support. The central conclusion is not simply that one model variant performed best, but that useful pharmacological decision support emerged only after combining structural representation improvement, real binding-data integration, hERG-focused refinement, probability calibration, fusion modeling, mechanistic simulation, and decision-oriented downstream analysis. The discussion below therefore follows the same thesis-essential sequence as the final results chapter rather than the broader exploratory notebook chronology.

\subsection{Structural Representation Is a Necessary Starting Point}
...
\subsection{Real Binding Data Provides the First Major Task-Specific Gain}
...
\subsection{Predictive Refinement and Calibration Must Precede Translation}
...
\subsection{Fusion Modeling Benefits from Complementary Structural Views}
...
\section{Mechanistic PK-PD Interpretation}
...
\section{Pareto, Monte Carlo, and Virtual Trial Implications}
...
\begin{enumerate}
  ...
  \item \textbf{SMILES availability}: the filtered hERG dataset used in the representation-upgrade stage (4,916 compounds vs. 7,997 total) reflects that not all compounds have associated canonical SMILES in the TDC/ChEMBL releases at the time of data collection.
  ...
  \item \textbf{GNN training data}: The GNN branch is trained only on binding affinity; the other three tasks retain MLP-only heads in the joint fusion model. Extension to multi-task GNN with graph inputs for all tasks is deferred to future work.
