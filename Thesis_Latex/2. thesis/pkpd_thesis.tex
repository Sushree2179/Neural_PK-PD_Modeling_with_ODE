...
% Latin Modern Roman is the XeLaTeX default; no \setmainfont needed
\fi
...
\hline
Chapter~2 & Reviews the background in ADME-Tox endpoint modeling, multi-task learning, graph neural networks, PK-PD ODE theory, and probability calibration. \\[4pt]
Chapter~3 & Surveys the related literature and identifies the research gap addressed by the work. \\[4pt]
Chapter~4 & Formally defines the multi-task learning problem, evaluation metrics, and scope. \\[4pt]
Chapter~5 & Describes the data sources, curation pipeline, feature engineering, and split strategy. \\[4pt]
Chapter~6 & Presents the full methodology, covering the shared encoder architecture, task-specific heads, Neural ODE PK-PD module, calibration procedures, and the RapidDock exploratory branch. \\[4pt]
Chapter~7 & Defines the experimental setup, environment, and training protocol. \\[4pt]
Chapter~8 & Reports the empirical results as a staged progression from baseline prediction to translational decision analysis. \\[4pt]
Chapter~9 & Discusses the findings, limitations, and comparisons with related work. \\[4pt]
Chapter~10 & Concludes with a summary and directions for future work. \\[4pt]
Appendix & Collects supporting tables, hyperparameter configurations, calibration figures, and reproducibility notes. \\
\hline
\end{tabular}
...
Absorption, distribution, metabolism, excretion, and toxicity (ADME-Tox) endpoints are central to candidate selection. This thesis models binding affinity, hERG-related cardiotoxicity risk, Caco-2 permeability, and clearance. See Appendix for endpoint definitions and full details.

\section{Multi-Task Learning}


Multi-task learning leverages shared representations to improve generalization across related endpoints~\cite{goodfellow2016}. See Appendix for architectural details.

\section{Graph Neural Networks for Molecular Modeling}


Molecules are represented as graphs, and message passing neural networks (MPNNs)~\cite{gilmer2017mpnn} are used for property prediction. See Appendix for details.

\section{PK-PD Modeling with ODEs}

PK-PD modeling connects drug concentration in the body to its biological effect using ODEs. See Appendix for full equations and parameter mapping.

\section{Calibration and Decision Reliability}


Probability calibration is required for classification tasks when outputs are used in downstream risk computations. This thesis evaluates Expected Calibration Error (ECE) and applies temperature scaling and Platt scaling~\cite{guo2017}. See Appendix for calibration details.
...
Regression tasks are evaluated with $R^2$ and RMSE. Classification tasks are evaluated with AUROC, F1, and accuracy, with calibrated probability quality measured by ECE. See Appendix~\ref{app:supplementary} for full metric definitions, equations, and research targets.
...
See Appendix~\ref{app:supplementary} for full data source descriptions and supplementary tables.
...
\noindent\textbf{hERG Inhibition (Cardiotoxicity Risk)}\par\medskip
...
\noindent\textbf{Caco-2 Permeability (Intestinal Absorption)}\par\medskip
...
\noindent\textbf{Hepatocyte Clearance (Metabolic Stability)}\par\medskip
...
Feature engineering combines molecular fingerprints, physicochemical descriptors, and graph representations. See Appendix~\ref{app:supplementary} for full feature engineering details and descriptor lists.
...
The feature engineering and model training pipeline are controlled through a centralized configuration. See Appendix~\ref{app:supplementary} for the full hyperparameter table and configuration details.
...
Quality control procedures ensure that model performance reflects genuine predictive signal rather than artifacts. See Appendix~\ref{app:supplementary} for full quality control and split strategy details.
...
Two fine-tuning strategies are evaluated:
\begin{itemize}
  \item \textbf{Heads-only fine-tuning}: All parameters except the two classification heads are frozen (SharedEncoder, PKODEFunc, DeepRegressionHead, RegressionHead frozen; \texttt{herg\_head} and \texttt{caco2\_head} trainable). Configuration: $\text{lr} = 3\times10^{-4}$, $\text{weight\_decay} = 10^{-5}$, 40 epochs, patience = 10. Scheduler monitors mean validation AUROC.
  \item \textbf{Partial encoder unfreeze}: Additionally unfreezes the final encoder block (PKODEFunc, regression heads, encoder blocks 0--7 frozen; encoder blocks 8--9, \texttt{herg\_head}, \texttt{caco2\_head} trainable). Configuration: $\text{lr} = 10^{-4}$, 60 epochs, patience = 12. The best by validation mean AUROC is selected as the calibration base model.
\end{itemize}

\subsection{Threshold Calibration}

\noindent\textbf{Unconstrained Threshold Sweep}\par\medskip
...
\noindent\textbf{Constrained Threshold Calibration}\par\medskip
...
\noindent\textbf{Production Threshold Policy}\par\medskip
...
The current feature engineering and data preparation notebook does not yet replace the exploratory ChEMBL binding subset with a real-SMILES binding file. Instead, it performs the following upgrade:
\begin{itemize}
...
Two fine-tuning strategies are evaluated: heads-only updates and selective encoder unfreezing. Threshold calibration is applied post-training for classification heads.
...
\noindent Classification tasks use focal loss ($\gamma=2.0$) to handle class imbalance. Regression tasks use mean squared error (MSE). The combined multi-task loss is a weighted sum (Equation~\ref{eq:mtl_loss}) with task-specific weights from Table~\ref{tab:hyperparam_summary}.


\section{Experimental Sequence}

The main experiment notebook program is organized as a staged progression from data realism, through predictive refinement, to mechanistic and translational evaluation. The core sequence is as follows:
\begin{enumerate}
  \item Real ChEMBL binding SMILES and full 2048-bit fingerprint integration: establishes the transition from the TDC-focused upgrade to a realistic binding-input representation.
  \item hERG AUROC improvement via hidden dimension expansion and positive-class weighting: defines the strongest toxicity-classification configuration for later analysis.
  \item Probability calibration using temperature scaling and Platt scaling with ECE evaluation: converts raw classifier outputs into decision-relevant risk probabilities.
  \item End-to-end joint GNN + MLP fusion model: provides the strongest structure-aware binding model and the final predictive architecture for downstream translation.
  \item Neural ODE PK/PD integration with population simulation: links predictive outputs to mechanistic exposure-response behavior.
  \item Pareto-front efficacy-safety analysis: replaces single-objective optimization with a clinically interpretable multi-objective trade-off framework.
  \item Monte Carlo and bootstrap uncertainty analysis: quantifies robustness of the recommended regimens and validates stability of the simulated conclusions.
  \item In-silico clinical trial simulation and operating characteristics: demonstrates how the final framework can support translational dose-selection decisions.
\end{enumerate}

Supporting and exploratory analyses (such as the representation-upgrade stage, standalone GNN ablation, scenario/sensitivity/single-objective optimization) are discussed as internal comparisons but are not emphasized in the main thesis argument.
...
See Appendix~\ref{app:supplementary} for the full table summarizing the molecular representation upgrade.
...
See Appendix~\ref{app:supplementary} for full performance summary and experiment progression figures.
...
See Appendix~\ref{app:supplementary} for representative Neural ODE PK-PD profiles.
...
See Appendix~\ref{app:supplementary} for calibration ECE figures.
...
See Appendix~\ref{app:supplementary} for the full Pareto analysis table.
...
See Appendix~\ref{app:supplementary} for the full virtual clinical trial summary table.

\section{Exploratory Extensions}


An exploratory RapidDock-inspired branch was implemented as a proof-of-feasibility study for geometry-aware protein-ligand modeling. See Appendix~\ref{app:supplementary} for full protocol and results.
...
The discussion chapter interprets the neural PK-PD framework as a staged progression from molecular prediction to translational decision support. Key findings and supporting details are summarized in the main text; see Appendix~\ref{app:supplementary} for extended discussion and supporting tables/figures.
