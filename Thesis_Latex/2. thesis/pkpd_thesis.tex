...
The thesis is organized as follows. Chapter~2 reviews the background in ADME-Tox endpoint modeling, multi-task learning, graph neural networks, PK-PD ODE theory, and probability calibration. Chapter~3 surveys the related literature and identifies the research gap addressed by the work. Chapter~4 formally defines the multi-task learning problem, evaluation metrics, and scope. Chapter~5 describes the data sources, curation pipeline, feature engineering, and split strategy. Chapter~6 presents the full methodology, covering the shared encoder architecture, task-specific heads, Neural ODE PK-PD module, calibration procedures, and the RapidDock exploratory branch. Chapter~7 defines the experimental setup, environment, and training protocol. Chapter~8 reports the empirical results as a staged progression from baseline prediction to translational decision analysis. Chapter~9 discusses the findings, limitations, and comparisons with related work. Chapter~10 concludes with a summary and directions for future work. The Appendix collects supporting tables, hyperparameter configurations, calibration figures, and reproducibility notes.
